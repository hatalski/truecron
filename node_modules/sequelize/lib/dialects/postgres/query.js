...
var fixFieldsCase = function(model, rows) {
  var attrsMap = Utils._.reduce(model.attributes, function (m, v, k) { m[k.toLowerCase()] = k; return m; }, {});
      ...
      if (self.isShowIndexesQuery()) {
        ...
        if (self.sql.toLowerCase().indexOf('select c.column_name') === 0) {
          ...
          if (self.options.raw === false && self.sequelize.options.quoteIdentifiers === false) {
            fixFieldsCase(self.callee, rows);
          }
        ...
        if (!!self.callee && self.options.raw === false && self.sequelize.options.quoteIdentifiers === false) {
          fixFieldsCase(self.callee.Model, rows);
        }
  ...
  Query.prototype.formatError = function (err) {
    ...
    switch (err.code) {
      case '23503':
        ...
        // error: insert or update on table "job" violates foreign key constraint "job_workspace_fk"
